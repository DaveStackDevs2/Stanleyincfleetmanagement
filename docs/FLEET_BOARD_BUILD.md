...
- `frontend/src/calendar/VehicleCalendar.tsx`

The exported component name remains temporarily unchanged to avoid breaking routing while the page is rebuilt. User-facing title is **Vehicle Availability** / **Fleet Board**. Rename folders/components only after imports and routing are inspected.
...
- [x] Add Available Now filter foundation.
- [x] Add click-vehicle action dialog foundation.
- [x] Add empty-time Quote/Reservation dialog foundation.
- [~] Replace old calendar CSS with Fleet Board CSS.
- [ ] Verify TypeScript build and correct all compile/runtime errors.
- [ ] Rename component/files/routes from Vehicle Calendar to Fleet Board after route inspection.
...
- [~] Read `public.reservations` separately for model-level scheduled reservations.
- [~] Read `public.rental_model_limits` for editable capacity values.
...
- [ ] Replace broad `select('*')` with explicit stable field lists.
- [ ] Create a board-state RPC/view only if required for permissions, performance, or consistent overlap logic.

### Reservation capacity

- [~] Show model/class capacity rows.
- [~] Calculate visible-period reservation usage.
- [ ] Calculate capacity per calendar day, not only total visible reservations.
...
- [~] Quote dialog entry point.
- [~] Reservation dialog entry point.
- [~] Rent Now entry point.
- [~] Loan Now entry point.
