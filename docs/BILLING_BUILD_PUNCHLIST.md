...
- [x] **IMPLEMENTED IN REPOSITORY:** `billing.extended_warranty_reconcile` is assigned to the existing Dev role by role name. The internal payload engine is restricted to `postgres` and `service_role`; the explicit AAL2 wrapper enforces active-user and effective-permission checks; and the non-AAL2 workspace orchestrator validates its timestamp, reconciles active warranty cases deterministically, then returns the unchanged authoritative workspace payload.
