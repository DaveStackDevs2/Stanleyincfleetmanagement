...
**IMPLEMENTED / NOT DEPLOYED / NOT LIVE-VERIFIED:** Original Rental charges now cover scheduled start through expected return without blocking pickup when Not Paid. Each original/Extension parent line has durable audited payment state; payment means an external Tekion Rental Sale charge. Unpaid active Rentals use the existing Warning Center Warning category. Rental Extension is independent from Loaner billed-through, creates a separate same-vehicle line, and does not reset Days in Vehicle. SO number and mass/bulk Loaner billed-through remain deferred.
- **CORRECTED LOCALLY / NOT DEPLOYED:** Pre-push audit defects are corrected in the existing pending migration: authoritative Extension-chain reuse, future-start preview, pickup payment/tax response, Rental Balance Due detail, active-only unpaid warnings with mutation reload, and Loaner preview-before-confirm are implemented. Live and browser verification remain pending.
