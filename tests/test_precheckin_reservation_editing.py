...
class PrecheckinReservationEditingTests(unittest.TestCase):
 def test_security_and_guards(self):
  ...
  self.assertRegex(SQL,r"lower\(btrim\(coalesce\(v_event\.status,''\)\)\)<>'active' or v_event\.closed_at is not null")
 ...
 def test_pickup_read_is_preserved_and_has_notes(self):
  ...
  self.assertIn('join public.rental_pricing_agreements a on a.reservation_id=r.id and a.transportation_event_id=r.transportation_event_id and a.is_active=true and a.pricing_started_at is null',read)
  self.assertIn("join public.transportation_events te on te.id=r.transportation_event_id and te.status='active'",read)
  self.assertIn("where r.status is distinct from 'cancelled' and r.actual_return_datetime is null",read)
