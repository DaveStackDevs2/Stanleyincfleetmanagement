...
class PrecheckinReservationEditingTests(unittest.TestCase):
 def test_security_and_guards(self):
  ...
  self.assertIn("if p_reservation_id is null then raise exception 'reservation is required' using errcode='22023'",SQL)
  self.assertIn("if p_start_date is null or p_expected_return_datetime is null then raise exception 'scheduled start and scheduled return are required' using errcode='22023'",SQL)
  self.assertIn("if lower(coalesce(v_reservation.status,''))='cancelled' or v_reservation.actual_return_datetime is not null",SQL)
  ...
  agreement_lock=SQL.index('select * into v_agreement')
  pricing_guard=SQL.index("raise exception 'reservation can no longer be edited after check-in'",agreement_lock)
  event_lock=SQL.index('select * into v_event',agreement_lock)
  self.assertLess(pricing_guard,event_lock)
  self.assertIn("raise exception 'reservation can no longer be edited after vehicle check-in' using errcode='p0001'",SQL)
  self.assertIn("raise exception 'reservation can no longer be edited after billing starts' using errcode='p0001'",SQL)
 ...
 def test_narrow_write_and_expected_return_engine(self):
  ...
  self.assertIn("raise exception 'expected-return engine did not update the transportation event';",SQL)
  self.assertNotIn("expected-return engine did not update the transportation event' using errcode",SQL)
  ...
  last_audit=SQL.rindex('insert into public.audit_log')
  self.assertGreater(SQL.index('update public.reservations set'),last_audit)
  self.assertGreater(SQL.index('v_expected:=public.set_expected_return_state'),last_audit)
  self.assertGreater(SQL.index('update public.transportation_events set notes='),last_audit)
 def test_audit_and_no_side_effects_or_fixtures(self):
  ...
  self.assertEqual(SQL.count('insert into public.audit_log'),5)
  self.assertNotIn('foreach v_field',SQL)
  self.assertIn("'changed_fields',v_audit_count",SQL)
 ...
 def test_frontend_authoritative_boundary(self):
  ...
  self.assertIn("submittedStart=startChanged?new Date(start).toISOString():originalStart",EDIT)
  self.assertIn("submittedReturn=expectedReturnChanged?new Date(expectedReturn).toISOString():originalExpectedReturn",EDIT)
  self.assertIn('p_start_date:submittedStart,p_expected_return_datetime:submittedReturn',EDIT)
