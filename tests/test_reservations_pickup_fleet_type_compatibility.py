...
class FleetCompatibilityMigrationTests(unittest.TestCase):
 def test_helper_boundary_and_guard(self):
  ...
  self.assertIn("v_reservation_type='rental' and v_vehicle_type<>'rental'",LOWER)
  self.assertIn("vehicle fleet type % is not eligible for reservation type %",LOWER);self.assertIn("errcode='22023'",LOWER)
  self.assertLess(LOWER.index("errcode='22023'"),LOWER.index('public.start_vehicle_use_state'))
  helper=LOWER.split('create or replace view public.v_reservation_vehicle_candidates',1)[0]
  self.assertIn('from public,anon,authenticated',helper);self.assertIn('to postgres,service_role',helper);self.assertNotIn('security definer',helper)
 def test_candidate_view(self):
  self.assertIn('with (security_invoker=true)',LOWER);self.assertIn('v.model = r.requested_model',LOWER);self.assertIn("lower(btrim(r.reservation_type))='loaner'",LOWER)
