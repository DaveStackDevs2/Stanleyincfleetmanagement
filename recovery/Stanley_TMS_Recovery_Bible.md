...
## 2026-08-19 — Verified Return/Complete final Billing persistence

Closed-case review revealed that Return/Complete could preserve stale final parent-line money when a multi-day case had not received a final manual checkpoint. The verified live `complete_case_return_and_close_state` now calls the existing authoritative preview at `p_actual_in_at`, validates the same open parent, stores preview subtotal/tax, reconciles the existing tax child, and only then runs the established return and close helpers. The function remains `postgres`-owned, `SECURITY INVOKER`, without a function `search_path` override, and restricted to `postgres` plus `service_role`; case completion does not require `billing.mark_billed_through`.

Read-only live verification used active multi-day TE `a5757d9d-8234-40bf-86e3-9d02d70e28dc` and billing line `db1c4f05-7c38-40a1-a0ae-13d463bfae95`: the stored checkpoint is `$400 + $40`, while the current authoritative 19-contract-day preview is `$760 + $76 = $836` and identifies the same current billing line. This did not mutate that production case. A production multi-day Return mutation/browser test remains the next verification after deployment/reconciliation. Closed Cases browser verification is also pending; the controlled closed Rental `$40 + $4 = $44` remains verified, with no closed Loaner browser verification yet.
...
- **VERIFIED PRODUCTION PASS:** The first repeated Extension correctly rejected before Tekion Billing advanced. The later 3:43 PM checkpoint correctly captured zero Extension days and `$0/$0`; its zero-tax-child removal is not a defect.
- **IMPLEMENTED IN REPOSITORY / NOT DEPLOYED:** The defect was the successor segment's relative contract-day clock. The preview-only follow-up measures each true Extension segment between positions on the Reservation's scheduled Billing clock and leaves checkpoint and wrapper definitions unchanged.
- **NOT VERIFIED / STILL PENDING:** Deployment and successful repeated-Extension production proof remain open. This correction has no browser verification, and Phase 8 Extension remains **not complete**.
