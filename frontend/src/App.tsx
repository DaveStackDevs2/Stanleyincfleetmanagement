...
const adminFeatures: AdminFeature[] = [
  {
    ...
    description:
      'View, search, add, edit, retire, reactivate, and review every fleet vehicle.',
    status: 'Foundation',
  },
  {
    title: 'GM OnTrac Sync Center',
    description:
      'Upload In-Service and Expiring Plates reports, preview changes, review errors, and maintain import history.',
    status: 'Foundation',
  },
  {
    title: 'CTP Threshold Settings',
    description:
      'Manage preferred and absolute day and odometer limits for CTP vehicles.',
    status: 'Foundation',
  },
  {
    title: 'Retirement Review',
    description:
      'Review vehicles missing from OnTrac and approve retirement with an effective date and reason.',
    status: 'Planned',
  },
  {
    title: 'Plate Administration',
    description:
      'Assign plates, track expiration dates, review plate changes, and preserve plate history.',
    status: 'Planned',
  },
  {
    title: 'Vehicle Import Review',
    description:
      'Review new vehicles, unmatched VINs, manual entries, duplicate candidates, and failed rows.',
    status: 'Planned',
  },
  {
    title: 'Users & Permissions',
    description:
      'Manage users, roles, access levels, account status, and Admin permissions.',
    status: 'Planned',
  },
  {
    title: 'Locations & Fleet Types',
    description:
      'Manage dealership locations, vehicle classifications, and fleet-use categories.',
    status: 'Planned',
  },
  {
  ...
  },
  {
    title: 'Notifications',
    description:
      'Manage alert rules, recipients, severity, delivery channels, and cooldown periods.',
    status: 'Planned',
  },
  {
    title: 'Audit & Activity History',
    description:
      'Review administrative changes, vehicle history, imports, approvals, and user activity.',
    status: 'Planned',
  },
  {
    title: 'System Settings',
    description:
      'Manage dealership-wide defaults, integrations, security settings, and operational rules.',
    status: 'Planned',
  },
  {
    title: 'Reports & Exports',
    description:
      'Run fleet, CTP, utilization, plate, retirement, rental, and exception reports.',
    status: 'Planned',
  },
  {
...
const fleetFilters: FleetFilter[] = [
  'All',
  'Active',
  'Retired',
  'Loaner',
...
function App() {
  ...
  useEffect(() => {
    ...
    const loadVehicles = async () => {
      ...
      try {
        ...
        const loadedVehicles = (data ?? []) as Vehicle[]
        ...
        setSelectedVehicleId((current) =>
          current &&
          loadedVehicles.some(
            (vehicle) => vehicle.vehicle_id === current,
          )
            ? current
  ...
  const statusCards: Array<[string, number]> = [
    ['Total Fleet', vehicles.length],
    [
      'Available',
      vehicles.filter(
        (vehicle) =>
          vehicle.vehicle_status?.toLowerCase() === 'available',
      ).length,
    ],
    [
      'Out',
      vehicles.filter((vehicle) =>
        vehicle.vehicle_status?.toLowerCase().includes('out'),
      ).length,
    ],
    [
      'Needs Review',
      vehicles.filter(
        (vehicle) =>
          vehicle.plate_sync_required || !vehicle.ontrac_last_seen_at,
      ).length,
    ],
    ['Over Preferred', 0],
    ['Over Maximum', 0],
          ...
          <main className="content">
            <section className="page-heading">
              <div>
                ...
                <h1>Admin Console</h1>
                <p>
                  These buttons establish the complete Admin Console
                  foundation. Features remain visible even before their
                  backend functionality is connected.
                </p>
              </div>
              ...
              <div className="sync-card">
                ...
                <small>
                  In-Service List and Expiring Plates required
                </small>
              </div>
            </section>
            ...
            <section
              ...
              aria-label="Admin Console features"
            >
              {adminFeatures.map((feature) => (
                ...
                >
                  <span
                    className={`status ${feature.status
                      .toLowerCase()
                      .replace(' ', '-')}`}
                  >
                    {feature.status}
          ...
          <main className="content fleet-page">
            <section className="fleet-header">
              <div>
                ...
                <p>
                  Live vehicle records from the Admin vehicle master
                  view.
                </p>
              </div>
            ...
            <section className="filter-panel">
              <div className="search-field">
                ...
                <input
                  ...
                  placeholder="VIN • Last 8 • Plate • Stock • Model • Trim"
                />
              </div>
            ...
            {loadError && (
              <div className="data-message error-message">
                <strong>Fleet data could not load.</strong>
                <span>{loadError}</span>
              </div>
            )}
                      ...
                      {!loading &&
                        filteredVehicles.map((vehicle) => (
                          ...
                          >
                            ...
                            <td>
                              {formatNumber(
                                vehicle.ontrac_days_in_service,
                              )}
                            </td>
                            <td>
                              {vehicle.record_source || '—'}
                            </td>
                          </tr>
                        ))}
              ...
              <aside className="details-panel">
                <div className="section-heading compact">
                  <div>
                    ...
                    <p>
                      {selectedVehicle
                        ? selectedVehicle.vin_last8
                        : 'Select a vehicle to review its information.'}
                    </p>
                  </div>
                </div>
                ...
                {selectedVehicle ? (
                  <dl className="vehicle-details-list">
                    ...
                    <div>
                      ...
                      <dd>
                        {selectedVehicle.vin ||
                          `Last 8: ${selectedVehicle.vin_last8}`}
                      </dd>
                    </div>
                    ...
                    <div>
                      <dt>Plate Expiration</dt>
                      <dd>
                        {formatDate(
                          selectedVehicle.plate_expiration_date,
                        )}
                      </dd>
                    </div>
                    ...
                    <div>
                      <dt>Odometer</dt>
                      <dd>
                        {formatNumber(selectedVehicle.odometer)}
                      </dd>
                    </div>

                    <div>
                      <dt>Qualified Miles</dt>
                      <dd>
                        {formatNumber(
                          selectedVehicle.qualified_miles,
                        )}
                      </dd>
                    </div>

                    <div>
                      <dt>Days In Service</dt>
                      <dd>
                        {formatNumber(
                          selectedVehicle.ontrac_days_in_service,
                        )}
                      </dd>
                    </div>
                    ...
                    <div>
                      <dt>Record Source</dt>
                      <dd>
                        {selectedVehicle.record_source || '—'}
                      </dd>
                    </div>

                    <div>
                      <dt>Last OnTrac Seen</dt>
                      <dd>
                        {formatDate(
                          selectedVehicle.ontrac_last_seen_at,
                        )}
                      </dd>
                    </div>
                    ...
                    <div>
                      <dt>Location</dt>
                      <dd>{selectedVehicle.location || '—'}</dd>
                    </div>
                  </dl>
                ) : (
            ...
            <section className="history-card">
              <div className="section-heading compact">
                <div>
                  ...
                  <p>
                    Plate, stock number, status, retirement, import,
                    and administrative history will appear here.
                  </p>
                </div>
              </div>
              ...
              <div className="history-placeholder">
                {selectedVehicle
                  ? `History connection pending for VIN ${selectedVehicle.vin_last8}.`
