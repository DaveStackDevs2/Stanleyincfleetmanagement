...
const fleetFilters: FleetFilter[] = ['All', 'Active', 'Retired', 'Loaner', 'Rental', 'Available', 'Out', 'Maintenance']
...
function App() {
  ...
  const filteredVehicles = useMemo(() => {
    ...
    return vehicles.filter((vehicle) => {
      const matchesSearch = !term || [vehicle.vin, vehicle.vin_last8, vehicle.license_plate, vehicle.stock_number, vehicle.model, vehicle.trim]
  ...
  const statusCards: Array<[string, number]> = [
    ...
    ['Needs Review', vehicles.filter((vehicle) => vehicle.plate_sync_required || !vehicle.ontrac_last_seen_at).length],
    ['Over Preferred', 0],
    ['Over Maximum', 0],
  ]
          ...
          <main className="content fleet-page">
            ...
            <section className="filter-panel">
              <div className="search-field">
                ...
                <input id="vehicle-search" type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="VIN • Last 8 • Plate • Stock • Model • Trim" />
              </div>
            ...
            <section className="fleet-layout">
              <div className="vehicle-table-card">
                <div className="section-heading"><div><h2>Fleet Vehicles</h2><p>{loading ? 'Loading vehicles…' : `${filteredVehicles.length} of ${vehicles.length} vehicles shown`}</p></div></div>
                <div className="table-wrap">
                  <table>
                    <thead><tr><th>Vehicle</th><th>VIN Last 8</th><th>Plate</th><th>Fleet Type</th><th>Status</th><th>Odometer</th><th>Days In Service</th><th>Source</th></tr></thead>
                    <tbody>
                      {!loading && filteredVehicles.map((vehicle) => (
                        <tr className={selectedVehicleId === vehicle.vehicle_id ? 'selected-row' : ''} onClick={() => setSelectedVehicleId(vehicle.vehicle_id)} key={vehicle.vehicle_id}>
                          ...
                          <td>{vehicle.vin_last8}</td><td>{vehicle.license_plate || '—'}</td><td>{vehicle.fleet_type || '—'}</td>
                          <td>{vehicle.is_retired ? 'Retired' : vehicle.vehicle_status || '—'}</td>
                          <td>{formatNumber(vehicle.odometer)}</td><td>{formatNumber(vehicle.ontrac_days_in_service)}</td><td>{vehicle.record_source || '—'}</td>
                        </tr>
                      ))}
                      {!loading && !loadError && filteredVehicles.length === 0 && <tr className="empty-row"><td colSpan={8}>No vehicles match the current search and filter.</td></tr>}
              ...
              <aside className="details-panel">
                <div className="section-heading compact"><div><h2>Vehicle Details</h2><p>{selectedVehicle ? selectedVehicle.vin_last8 : 'Select a vehicle to review its information.'}</p></div></div>
                {selectedVehicle ? (
            ...
            <section className="history-card">
              <div className="section-heading compact"><div><h2>Vehicle History</h2><p>Plate, stock number, status, retirement, import, and administrative history will appear here.</p></div></div>
