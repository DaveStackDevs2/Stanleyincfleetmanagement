...
create or replace function public.create_authoritative_start_bill_case_state(
  ...
  p_transportation_type text, p_ro_number text, p_pay_type text,
  p_notes text, p_service_advisor text, p_start_mileage integer
) returns jsonb
language plpgsql security definer set search_path to ''
as $function$
declare
  v_actor uuid; v_create jsonb; v_start jsonb; v_preview jsonb; v_line jsonb;
...
begin
  ...
  v_create := public.create_reservation_with_transportation_event_state(
    p_start_date, p_expected_return_datetime,
    (select v.model from public.vehicles v where v.id=p_vehicle_id),
    p_transportation_type, 'active', p_notes, p_customer_id,
    p_service_advisor, p_ro_number, p_pay_type, p_vehicle_id);
  v_reservation := (v_create->>'reservation_id')::uuid;
  v_event := (v_create->>'transportation_event_id')::uuid;
  if v_reservation is null or v_event is null then raise exception 'Operational case engine returned incomplete identifiers'; end if;

  v_start := public.start_vehicle_use_state(v_event, p_vehicle_id, p_actual_out_at);
  v_vehicle_event := (v_start->>'vehicle_event_id')::uuid;
  v_period := (v_start->>'contract_period_id')::uuid;
  update public.reservations set start_mileage=p_start_mileage, status='active' where id=v_reservation;
  ...
  v_preview := public.get_billing_preview_state(v_event, p_actual_out_at);
  ...
  v_line := public.create_billing_parent_line_state(
    ...
    p_actual_out_at, null, v_preview->>'rate_source', v_vehicle_event, v_period,
    'initial_assignment', null, null, null, true, null, null,
    (v_preview->>'daily_rate')::numeric, null);
  v_line_id := (v_line->>'parent_billing_line_id')::uuid;
  ...
  v_final := public.get_billing_preview_state(v_event, p_actual_out_at);
...
begin
  ...
  if p_billed_through_at > clock_timestamp() then raise exception using errcode='22023', message='Billed-through timestamp cannot be in the future'; end if;
  ...
  select count(*) into v_open_count from public.billing_lines bl where bl.reservation_id=p_reservation_id and bl.parent_billing_line_id is null and bl.is_open=true;
  if v_open_count <> 1 then raise exception using errcode='21000', message='Reservation must have exactly one open parent billing segment'; end if;
  select * into v_line from public.billing_lines bl where bl.reservation_id=p_reservation_id and bl.parent_billing_line_id is null and bl.is_open=true for update;
