...
declare v_actor uuid; v_event public.transportation_events%rowtype; v_rule record; v_ext_pay public.pay_type_rules%rowtype; v_customer_pay public.pay_type_rules%rowtype; v_pickup timestamptz; v_line public.billing_lines%rowtype; v_case public.warranty_cases%rowtype; begin
  ...
  values(p_transportation_event_id,v_line.reservation_id,v_rule.provider_id,v_rule.provider_name,v_rule.rule_id,v_rule.covered_days,v_rule.resolved_daily_rate,v_pickup,v_customer_pay.id,case when v_rule.requires_approval then 'manual_review' else 'approved' end,null,clock_timestamp(),jsonb_build_object('created_by',v_actor,'provider_type','extended_warranty','case_created_from','first_vehicle_pickup')) returning * into v_case;
  update public.billing_lines set warranty_provider_id=v_rule.provider_id, default_covered_days_snapshot=v_rule.covered_days, covered_days_override=null, default_daily_rate_snapshot=v_rule.resolved_daily_rate where id=v_line.id;
...
declare v_actor uuid; v_case public.warranty_cases%rowtype; v_old jsonb; v_new jsonb; v_pay public.pay_type_rules%rowtype; begin
  ...
  v_old := jsonb_build_object('approved_days',v_case.approved_days,'effective_approved_days',coalesce(v_case.approved_days,v_case.default_covered_days_snapshot),'post_coverage_pay_type_rule_id',v_case.post_coverage_pay_type_rule_id);
  ...
  update public.warranty_cases set approved_days=p_covered_days_override, post_coverage_pay_type_rule_id=v_pay.id, override_reason=btrim(p_reason), override_authorized_by=v_actor, override_authorized_at=clock_timestamp() where id=p_warranty_case_id returning * into v_case;
  update public.billing_lines set covered_days_override=coalesce(v_case.approved_days,v_case.default_covered_days_snapshot) where transportation_event_id=v_case.transportation_event_id and pay_type='Extended Warranty' and parent_billing_line_id is null;
  v_new := jsonb_build_object('approved_days',v_case.approved_days,'effective_approved_days',coalesce(v_case.approved_days,v_case.default_covered_days_snapshot),'post_coverage_pay_type_rule_id',v_case.post_coverage_pay_type_rule_id);
  ...
  return jsonb_build_object('status','extended_warranty_case_override_saved','case_id',v_case.id,'approved_days',v_case.approved_days,'effective_approved_days',coalesce(v_case.approved_days,v_case.default_covered_days_snapshot),'post_coverage_pay_type_rule_id',v_case.post_coverage_pay_type_rule_id,'override_authorized_by',v_case.override_authorized_by,'override_reason',v_case.override_reason);
end;$function$;
...
declare v_case public.warranty_cases%rowtype; v_days integer; v_parent public.billing_lines%rowtype; v_split_count integer; v_boundary timestamptz; v_current_vehicle_event public.vehicle_events%rowtype; v_create jsonb; begin
  ...
  if v_case.coverage_started_at is null or coalesce(v_case.approved_days,v_case.default_covered_days_snapshot) is null then return jsonb_build_object('status','extended_warranty_no_cap_configured','manual_review',false,'case_id',v_case.id); end if;
  ...
  if p_effective_at < v_boundary then return jsonb_build_object('status','extended_warranty_coverage_active','case_id',v_case.id,'continuous_case_contract_day',v_days,'covered_days',coalesce(v_case.approved_days,v_case.default_covered_days_snapshot)); end if;
