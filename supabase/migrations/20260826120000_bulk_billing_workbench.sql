...
declare actor uuid;b public.billing_bulk_batches%rowtype;i public.billing_bulk_items%rowtype;current_r jsonb;current_l jsonb;current_w jsonb;s jsonb;old public.billing_lines%rowtype;old_w public.warranty_cases%rowtype;begin
 actor:=public.bulk_billing_actor_state();select * into b from public.billing_bulk_batches where actor_user_id=actor and status in('applied','partially_applied') order by applied_at desc limit 1 for update;if not found then raise exception 'No applicable Bulk Updating batch is available to Undo';end if;
 for i in select * from public.billing_bulk_items where batch_id=b.id and succeeded order by id for update loop
  ...
  if current_r is distinct from i.after_reservation or current_l is distinct from i.after_lines or (current_w-'current_day_count'-'last_checked_at'-'updated_at') is distinct from (i.after_warranty-'current_day_count'-'last_checked_at'-'updated_at') then raise exception 'Undo refused: legitimate Billing work changed after this Bulk batch for %',i.identifier;end if;
