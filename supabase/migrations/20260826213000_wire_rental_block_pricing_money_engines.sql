...
BEGIN
    ...
    IF v_preview_end < v_billing_start THEN
        IF v_is_rental AND lower(coalesce(v_current_line.line_type, '')) = 'rental_extension' THEN
            -- An accepted future Rental Extension has no completed days yet.
    ...
    SELECT coalesce(
        jsonb_agg(
            jsonb_build_object(
                ...
                'contract_days', CASE WHEN parent.start_time IS NULL THEN NULL ELSE public.business_contract_days(parent.start_time, coalesce(parent.end_time, parent.paid_through_at, v_event.closed_at, p_effective_at)) END,
