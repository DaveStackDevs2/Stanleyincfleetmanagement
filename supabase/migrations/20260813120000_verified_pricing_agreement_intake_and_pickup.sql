...
begin
 ...
 return jsonb_build_object(
  ...
  'customers',coalesce((select jsonb_agg(jsonb_build_object('customer_id',c.id,'tekion_customer_number',c.tekion_customer_number,'name',c.name,'phone',c.phone,'email',c.email,'created_at',c.created_at) order by c.name,c.id) from public.customers c),'[]'::jsonb),
  'pay_types',coalesce((select jsonb_agg(jsonb_build_object('pay_type_rule_id',p.id,'pay_type',p.pay_type,'description',p.description,'is_taxable',p.is_taxable,'sort_order',p.sort_order) order by p.sort_order,p.pay_type,p.id) from public.pay_type_rules p where p.is_active=true and coalesce(p.active,false)=true),'[]'::jsonb),
  ...
  'quotes',coalesce((select jsonb_agg(jsonb_build_object(
    ...
    'weekly_rate',a.weekly_rate_snapshot::text,'monthly_rate',a.monthly_rate_snapshot::text,'pricing_started_at',a.pricing_started_at) order by q.start_date,q.id)
...
begin
 ...
 select coalesce(jsonb_agg(jsonb_build_object(
   ...
   'pricing_agreement',jsonb_build_object('pricing_agreement_id',a.id,'origin_type',a.origin_type,'vehicle_class',a.vehicle_class,
     'rental_rate_rule_id',a.rental_rate_rule_id,'pay_type_rule_id',a.pay_type_rule_id,'pay_type',p.pay_type,'initial_rate_plan',a.initial_rate_plan,'current_rate_plan',a.current_rate_plan,
   ...
   'vehicle_candidates',coalesce((select jsonb_agg(jsonb_build_object(
        'vehicle_id',vc.vehicle_id,'vin',vc.vin,'stock_number',vc.stock_number,'vehicle_model',vc.vehicle_model,
...
declare
 ...
 v_current_vehicle_id uuid; v_existing_vehicle_event uuid; v_existing_contract_period uuid; v_existing_line uuid;
begin
 select id into v_user from public.app_users where auth_user_id=auth.uid() and is_active=true;
 if v_user is null then raise exception 'Active application user required' using errcode='42501'; end if;
 if coalesce(auth.jwt()->>'aal','')<>'aal2' then raise exception 'AAL2 authentication required' using errcode='42501'; end if;
 ...
 select c.vehicle_id,c.vehicle_event_id,c.contract_period_id into v_current_vehicle_id,v_existing_vehicle_event,v_existing_contract_period
 from public.v_current_vehicle_continuity c where c.transportation_event_id=v_reservation.transportation_event_id;
