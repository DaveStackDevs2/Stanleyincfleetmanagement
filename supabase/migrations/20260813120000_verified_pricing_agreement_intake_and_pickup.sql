...
begin
 ...
 if v_line->>'status'<>'parent_billing_line_created' then raise exception 'Billing engine did not create the initial billing line' using errcode='P0001'; end if;
 begin v_line_id:=(v_line->>'parent_billing_line_id')::uuid; exception when invalid_text_representation then raise exception 'Billing engine returned a malformed billing-line identifier' using errcode='P0001'; end;
 if v_line_id is null then raise exception 'Billing engine did not return a billing-line identifier' using errcode='P0001'; end if;
