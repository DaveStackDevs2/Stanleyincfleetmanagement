...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'rental_rate_rule_id', r.id, 'vehicle_class', r.vehicle_class,
    'pay_type_rule_id', r.pay_type_rule_id, 'pay_type', ptr.pay_type,
    'daily_rate', r.daily_rate, 'sort_order', r.sort_order,
    'is_active', r.is_active,
    'is_current', r.is_active and r.effective_from <= v_observed_at and (r.effective_to is null or r.effective_to > v_observed_at),
    'effective_from', r.effective_from, 'effective_to', r.effective_to,
    'created_at', r.created_at, 'updated_at', r.updated_at)
    order by lower(btrim(r.vehicle_class)), ptr.sort_order, ptr.pay_type, r.effective_from desc, r.id), '[]'::jsonb)
...
returns jsonb language plpgsql security definer set search_path to '' as $function$
declare v_user_id uuid; v_rule public.rental_rate_rules%rowtype; v_observed_at timestamptz;
begin
  select au.id into v_user_id from public.app_users au where au.auth_user_id = auth.uid() and au.is_active = true;
  if v_user_id is null or not exists (select 1 from public.v_user_effective_permissions p where p.user_id = v_user_id and p.permission_key = 'user_admin.manage') then raise exception 'Rental rate administration access denied' using errcode = '42501'; end if;
  ...
  return jsonb_build_object('status','admin_rental_rate_rule_created','rental_rate_rule',jsonb_build_object('rental_rate_rule_id', v_rule.id, 'vehicle_class', v_rule.vehicle_class, 'pay_type_rule_id', v_rule.pay_type_rule_id, 'pay_type', (select ptr.pay_type from public.pay_type_rules ptr where ptr.id = v_rule.pay_type_rule_id), 'daily_rate', v_rule.daily_rate, 'sort_order', v_rule.sort_order, 'is_active', v_rule.is_active, 'is_current', v_rule.is_active and v_rule.effective_from <= v_observed_at and (v_rule.effective_to is null or v_rule.effective_to > v_observed_at), 'effective_from', v_rule.effective_from, 'effective_to', v_rule.effective_to, 'created_at', v_rule.created_at, 'updated_at', v_rule.updated_at));
...
returns jsonb language plpgsql security definer set search_path to '' as $function$
declare v_user_id uuid; v_existing public.rental_rate_rules%rowtype; v_rule public.rental_rate_rules%rowtype; v_pay_enabled boolean; v_observed_at timestamptz;
begin
  select au.id into v_user_id from public.app_users au where au.auth_user_id = auth.uid() and au.is_active = true;
  if v_user_id is null or not exists (select 1 from public.v_user_effective_permissions p where p.user_id = v_user_id and p.permission_key = 'user_admin.manage') then raise exception 'Rental rate administration access denied' using errcode = '42501'; end if;
  ...
  v_observed_at := clock_timestamp();
  return jsonb_build_object('status','admin_rental_rate_rule_updated','rental_rate_rule',jsonb_build_object('rental_rate_rule_id', v_rule.id, 'vehicle_class', v_rule.vehicle_class, 'pay_type_rule_id', v_rule.pay_type_rule_id, 'pay_type', (select ptr.pay_type from public.pay_type_rules ptr where ptr.id = v_rule.pay_type_rule_id), 'daily_rate', v_rule.daily_rate, 'sort_order', v_rule.sort_order, 'is_active', v_rule.is_active, 'is_current', v_rule.is_active and v_rule.effective_from <= v_observed_at and (v_rule.effective_to is null or v_rule.effective_to > v_observed_at), 'effective_from', v_rule.effective_from, 'effective_to', v_rule.effective_to, 'created_at', v_rule.created_at, 'updated_at', v_rule.updated_at));
exception when unique_violation then raise exception 'A current rental rate already exists for this vehicle class and pay type' using errcode='23505';
end;$function$;
...
returns jsonb language plpgsql security definer set search_path to '' as $function$
declare v_user_id uuid; v_existing public.rental_rate_rules%rowtype; v_rule public.rental_rate_rules%rowtype; v_pay_enabled boolean; v_effective_at timestamptz; v_observed_at timestamptz;
begin
  select au.id into v_user_id from public.app_users au where au.auth_user_id = auth.uid() and au.is_active = true;
  if v_user_id is null or not exists (select 1 from public.v_user_effective_permissions p where p.user_id = v_user_id and p.permission_key = 'user_admin.manage') then raise exception 'Rental rate administration access denied' using errcode = '42501'; end if;
  if p_rental_rate_rule_id is null then raise exception 'Rental rate rule ID is required' using errcode='22023'; end if;
  ...
  update public.rental_rate_rules set is_active=p_is_enabled, effective_to=case when p_is_enabled then null when effective_to is not null then effective_to when v_effective_at <= effective_from then effective_from + interval '1 microsecond' else v_effective_at end, effective_from=case when p_is_enabled and not is_active then v_effective_at else effective_from end, updated_by=v_user_id where id=p_rental_rate_rule_id returning * into v_rule;
  v_observed_at := clock_timestamp();
  return jsonb_build_object('status', case when p_is_enabled then 'admin_rental_rate_rule_enabled' else 'admin_rental_rate_rule_disabled' end, 'rental_rate_rule', jsonb_build_object('rental_rate_rule_id', v_rule.id, 'vehicle_class', v_rule.vehicle_class, 'pay_type_rule_id', v_rule.pay_type_rule_id, 'pay_type', (select ptr.pay_type from public.pay_type_rules ptr where ptr.id = v_rule.pay_type_rule_id), 'daily_rate', v_rule.daily_rate, 'sort_order', v_rule.sort_order, 'is_active', v_rule.is_active, 'is_current', v_rule.is_active and v_rule.effective_from <= v_observed_at and (v_rule.effective_to is null or v_rule.effective_to > v_observed_at), 'effective_from', v_rule.effective_from, 'effective_to', v_rule.effective_to, 'created_at', v_rule.created_at, 'updated_at', v_rule.updated_at));
exception when unique_violation then raise exception 'A current rental rate already exists for this vehicle class and pay type' using errcode='23505';
end;$function$;
...
declare v_vehicle_class text; v_pay_type public.pay_type_rules%rowtype; v_match record;
begin
  if p_vehicle_class is null or btrim(p_vehicle_class) = '' then raise exception 'Vehicle class cannot be blank' using errcode='22023'; end if;
  if p_pay_type_rule_id is null then raise exception 'Pay type rule ID is required' using errcode='22023'; end if;
  if p_effective_at is null then raise exception 'Effective timestamp is required' using errcode='22023'; end if;
  v_vehicle_class := btrim(p_vehicle_class);
  select * into v_pay_type from public.pay_type_rules p where p.id = p_pay_type_rule_id;
  if not found then
    return jsonb_build_object('status','rental_daily_rate_pay_type_not_found','requested_vehicle_class',v_vehicle_class,'pay_type_rule_id',p_pay_type_rule_id,'effective_at',p_effective_at);
  end if;
  select r.*, v_pay_type.pay_type, (v_pay_type.is_active and coalesce(v_pay_type.active,false)) pay_type_is_enabled into v_match
  from public.rental_rate_rules r
  where lower(btrim(r.vehicle_class)) = lower(v_vehicle_class)
    and r.pay_type_rule_id = p_pay_type_rule_id
  ...
  order by r.effective_from desc, r.updated_at desc, r.id
  limit 1;
  if not found then
    return jsonb_build_object('status','rental_daily_rate_not_configured','requested_vehicle_class',v_vehicle_class,'pay_type_rule_id',p_pay_type_rule_id,'pay_type',v_pay_type.pay_type,'pay_type_is_enabled',v_pay_type.is_active and coalesce(v_pay_type.active,false),'effective_at',p_effective_at);
  end if;
  return jsonb_build_object('status','rental_daily_rate_resolved','requested_vehicle_class',v_vehicle_class,'rental_rate_rule_id',v_match.id,'vehicle_class',v_match.vehicle_class,'pay_type_rule_id',v_match.pay_type_rule_id,'pay_type',v_match.pay_type,'pay_type_is_enabled',v_match.pay_type_is_enabled,'daily_rate',v_match.daily_rate,'effective_at',p_effective_at,'effective_from',v_match.effective_from,'effective_to',v_match.effective_to);
end;$function$;
