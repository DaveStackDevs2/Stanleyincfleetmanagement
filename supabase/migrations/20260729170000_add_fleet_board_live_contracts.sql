-- Mirrors the Fleet Board contracts already applied to the live project.
...
begin
  ...
  return jsonb_build_object(
    ...
    'vehicles', coalesce((
      ...
      from (
        select id, stock_number, model, fleet_type, status, location
    ...
    'assignments', coalesce((
      ...
      from (
        ...
        where vehicle_id is not null
          ...
          and coalesce(actual_in_at, expected_return_at, current_billing_end_time) > p_range_start
      ) a
    ), '[]'::jsonb),
    'reservations', coalesce((
      select jsonb_agg(to_jsonb(r) order by r.start_date, r.id)
      from (
        select id, vehicle_id, start_date, expected_return_datetime, status, requested_model
...
create or replace function public.set_fleet_board_pay_type_colors_state(p_pay_type_colors jsonb)
returns jsonb
language plpgsql
security definer
set search_path = public, pg_temp
...
begin
  ...
  if p_pay_type_colors is null or jsonb_typeof(p_pay_type_colors) <> 'object' or exists (
    select 1
    from jsonb_each(p_pay_type_colors) entry
    where jsonb_typeof(entry.value) <> 'object'
      or not (entry.value ->> 'background_color' ~ '^#[0-9A-Fa-f]{6}$')
      or not (entry.value ->> 'text_color' ~ '^#[0-9A-Fa-f]{6}$')
  ) then
    raise exception 'Pay-type colors must be an object of six-digit hex color pairs' using errcode = '22023';
  end if;
  update public.admin_settings
  set setting_value = p_pay_type_colors
  where setting_key = 'fleet_board.pay_type_colors';
  return jsonb_build_object(
    'status', 'fleet_board_pay_type_colors_updated',
    'pay_type_colors', p_pay_type_colors
  );
end;
$$;
